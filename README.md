...
- In the root folder of your client app, run `npm link qway-vue`. This will add the symbolic link to the `node_modules` folder in your client app.
- You can now import `qway-vue` in your client app.

There is no need to add `qway-vue` to your client app's dependency in this case.
...
```js
<script lang="ts">
import { MyConstants, MyUtil } from 'qway-vue'
